...
// Phase 10
// doesn't work quite right, it keeps passing the error back over and over, although it does detect the page fault error types
...
int main() {
	...
	pid = DeQ(&free_q); //call DeQ() to dequeue free_q to get pid 2 (FileService)
	StartProcISR(pid, (int) FileService);
...
//	for(i=0; i<505; i++) {
//		MyBzero((char *)&pf_q[i], sizeof(TF_t));
//	}
	...
	for(i=0; i<MAX_PROC_NUM; i++){
        ...
        for(i=0; i<DRAM_PAGE_COUNT; i++) {
                DRAM[i].addr=dram_addr;
                dram_addr = dram_addr + 0x1000; //intervals of 4096
                DRAM[i].owner = -1;
